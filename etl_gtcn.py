...
etl_args = {
    "dest_table_name" : "air_etl_gtcn",
    "dest_table_columns_definition" : {
        "BRA_CODE":("Number(4)", int), 
        "CUS_NUM":("Number(7)", int), 
        "GTCN_TRA_COUNT":("Number", int), 
        "GTCN_TRA_AMT":("Number", float)}, # highlights destination table fields excluding reference_dates and control_dates; simply agg_columns and agg_measures
    "source_query" : """
        select bra_code, cus_num, gtcn_tra_amt, tra_date  
        from stg.src_transact
        where doc_alp = 'GTCN' and deb_cre_ind = 1 
        and cus_num > 99999
    ...
    "agg_measures" : [
        ("COUNT(1)","GTCN_TRA_COUNT"),
        ("NVL(SUM(TRA_AMT),0)", "GTCN_TRA_AMT")],
