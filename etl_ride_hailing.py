...
etl_args = {
    "dest_table_name" : "air_etl_ride_hailing",
    ...
    "source_query" : """
       select bra_code, cus_num, tra_amt, tra_date from stg.src_transact
        where 
         (upper(remarks) like '%UBER%' 
        or upper(remarks) like '%BOLT%'
        or upper(remarks) like '%BOLT%PAY%'
        or upper(remarks) like '%BOLT%EU%'
        or upper(remarks) like '%TAXIFY%'
        or upper(remarks) like '%GIDI%CAB%'
        or upper(remarks) like '%OGA%TAXI%'
        or upper(remarks) like '%JEKALO%'
        or upper(remarks) like '%RIDE%ON%'
        or upper(remarks) like '%RIDEME%TAXI%'
        ) 
        and not (upper(remarks) like '%UBER%EATS%' or upper(remarks) like '%HUBER%' or upper(remarks) like '%SUBERU%' 
        or upper(remarks) like '%ZUBERU%' or upper(remarks) like '%BUBERRY%')
        and deb_cre_ind = 1
