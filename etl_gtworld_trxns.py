...
dag = DAG(
    dag_id='air_etl_gtworld_trxns',
    default_args=args,
    schedule_interval=None,
    tags=['gtworld','transactions'],
)

etl_args = {
    "dest_table_name" : "air_etl_gtworld_trxns",
    "dest_table_columns_definition" : {
        "BRA_CODE":("Number(4)", int), 
        "CUS_NUM":("Number(7)", int), 
        "TRA_COUNT":("Number", int), 
        "TRA_AMT":("Number", float),
        "CHANNEL":("Varchar(20)", object)}, # highlights destination table fields excluding reference_dates and control_dates; simply agg_columns and agg_measures
    "source_query" : """
        select bra_code, cus_num, 'GTWORLD' as channel,  tra_amt, tra_date  
        from
        stg.src_transact
        where cus_num>99999 and deb_cre_ind=1
        and can_rea_code=0
        and (remarks like '%GTWORLD%' or doc_alp ='NIPG')
        and cur_code = 1
        and expl_code not in (44,58,84,95,100,103,139,147,148,156,157,
        205,330,336,340,471,541,645,903,917,964,965,225,644,993)
