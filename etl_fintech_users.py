...
dag = DAG(
    dag_id='air_etl_fintech_users',
    default_args=args,
    schedule_interval="@weekly",
    tags=['fintech','loans','investments'],
)

etl_args = {
    "dest_table_name" : "air_etl_fintech_users",
    ...
    "source_query" : """
       select bra_code, cus_num, tra_amt, tra_date from stg.src_transact
        where 
        (upper(remarks) like '%PIGGY%VEST%' 
        or upper(remarks) like '%PIGGY%BANK%'
        or upper(remarks) like '%PAGA%'
        or upper(remarks) like '%PAGA%TECH%'
        or upper(remarks) like '%CROWDY%VEST%'
        or upper(remarks) like '%REN%MONEY%'
        or upper(remarks) like '%COWRY%WISE%'
        or upper(remarks) like '%THRIVE%AGRIC%'
        or upper(remarks) like '%FARM%CROWDY%'
        or upper(remarks) like '%WEALTH.NG%'
        or upper(remarks) like '%WEALTH%NG%'
        or upper(remarks) like '%INTER%SWITCH%'
        or upper(remarks) like '%NAIJA%FUND%'
        or upper(remarks) like '%QUIDAX%'
        or upper(remarks) like '%PAYSTACK%'
        or upper(remarks) like '%FINT.NG%'
        or upper(remarks) like '%PAY%LATER%'
        or upper(remarks) like '%KONGA%PAY%'
        or upper(remarks) like '%ZED%VANCE%'
        or upper(remarks) like '%KWIK%MONEY%'
        ) 
        and deb_cre_ind = 1
