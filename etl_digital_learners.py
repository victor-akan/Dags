...
dag = DAG(
    dag_id='air_etl_digital_learners',
    default_args=args,
...
etl_args = {
    "dest_table_name" : "air_etl_digital_learners",
    ...
    "source_query" : """
        ...
        where   expl_code = 470 and 
        (upper(remarks) like '%UDEMY%'  
        or 
        upper(remarks) like '%EDX%USD%' 

        or 
        upper(remarks) like '%CODE%MOSH%' 
        or 
        upper(remarks) like '%UDACITY%' 
        or 
        upper(remarks) like '%DATA%CAMP%' 
        or 
        upper(remarks) like '%CODE%CADEMY%' 
        or upper(remarks) like '%ALISON%GALWAY%'
        or upper(remarks) like '%SKILL%SHARE%' 
        )
        AND CAN_REA_CODE = 0
